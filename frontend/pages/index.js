...
export default function Login({cookie}) {
  ...
  const loginSubmit = async () => {
    let data = await axios.get(backAddress + '/login/logincheck/' +  userId, {
      withCredentials: true
    })
    const tmp = data.data
    const tokendecode = jwt.decode(tmp.data)
  ...
  return (
      ...
      LOGIN FORM
      {
        loginState == 'Guest' ? <div>로그인 되어있지 않습니다.</div> : <div>{loginState} 님 반갑습니다.</div>
      }
      <input 
